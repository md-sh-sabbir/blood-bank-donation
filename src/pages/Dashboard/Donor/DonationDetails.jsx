...
const DonationDetails = () => {
    ...
    const [isLoading, setIsLoading] = useState(!state); // fetch only if no state

    // If no location state (e.g. came from notification popup), fetch by ID
    useEffect(() => {
        if (!state && id) {
            setIsLoading(true);
            axiosSecure
            ...
            <div className="flex justify-center items-center min-h-[calc(100vh-82px)] p-4">
                <div className="bg-white md:w-4/5 lg:w-3/5 rounded-xl shadow-lg p-6 hover:shadow-xl transition-shadow duration-300">
                    ...
                    <div className='mb-5'>
                        ...
                        <span>{donationData.requesterNumber}</span>
                    </div>
